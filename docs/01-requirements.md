...
| FR-011 | The system shall expose the following API endpoints: `/health`, `/ingest/documents`, `/ingest/sources`, `/query`, `/agents/run`, `/agents/tools`, `/vision/analyze`, `/video/analyze`, `/metrics`. | Contract tests confirming endpoint availability, status codes, and schema conformance. |
